...
// Importar configuraciones centralizadas - VERSIÓN CORREGIDA
...
class Member {
  constructor(name, email, preferences = []) {
    ...
    this.balance = 300; // Saldo inicial de 300€ (mayor para seguros)
  ...
  reduceBalance(amount, reason) {
    ...
    // Verificar challenges de multi-seguros
    this._checkMultiInsuranceChallenge(reason);
    
    return this.balance;
  }

  // Método específico para verificar el challenge de multi-seguros
  _checkMultiInsuranceChallenge(reason) {
    // Buscar productos de diferentes categorías en las transacciones
    const insuranceCategories = new Set();
    
    // Revisar transacciones de compra para contar categorías diferentes
    this.transactions.filter(tx => tx.type === 'Purchase').forEach(tx => {
      catalogConfig.products.forEach(product => {
        if (tx.description.includes(product.name)) {
          insuranceCategories.add(product.category);
        }
      });
    });
    
    // Actualizar progreso del challenge multi_seguros
    const multiInsuranceChallenge = this.challenges.find(c => c.id === 'multi_seguros');
    if (multiInsuranceChallenge && !multiInsuranceChallenge.completed) {
      multiInsuranceChallenge.progress = insuranceCategories.size;
      
      if (multiInsuranceChallenge.progress >= multiInsuranceChallenge.goal) {
        multiInsuranceChallenge.completed = true;
        this.addPoints(multiInsuranceChallenge.reward, `Reto completado: ${multiInsuranceChallenge.name}`);
        
        // Desbloquear logro relacionado
        const relatedAchievement = catalogConfig.achievements.find(a => 
          a.id === 'challenge_multi_seguros'
        );
  ...
  _checkTierUpdate() {
    ...
    if (newTierName !== this.tier) {
      ...
      this._addTransaction('System', `Actualización a nivel ${currentTierObj.displayName}`, 0, 0, 0, 0);
  ...
  getNextTier() {
    ...
    return nextTierObj ? nextTierObj.displayName : null;
  }
  ...
  // Obtener el nombre de display del tier actual
  getCurrentTierDisplayName() {
    const currentTierObj = tiersConfig.getTierByName(this.tier);
    return currentTierObj ? currentTierObj.displayName : this.tier;
  }
